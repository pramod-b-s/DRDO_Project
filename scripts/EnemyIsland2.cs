...
public class EnemyIsland2 : MonoBehaviour {

    public static int HealthIsland2 = 100;

    //dodgetime = diff in PlayerControl
    public float DodgeTime = 5;
    public Rigidbody islandEnemy2, sub;
    ...
    public float timeDelay2 = 120;

    private float nextTimeDownwardForce, DownwardForceFlag = 0;

    public static Vector3 DestroyedAtPosIsl2;
    public static float DestroyedAtTimeIsl2;
    private float var;

    // Use this for initialization
    void Start()
    {
        initPos = islandEnemy2.transform.position;
    }

    // Update is called once per frame
    void FixedUpdate()
    {
        if (Vector3.Distance(islandEnemy2.transform.position, initPos) <= 10 && RemTimeFlag == 0)
        {
            RemTimeFlag = 1;
        ...
        if (nextRemTime >= timeDelay2)
        {
            RemTime.text = "Habitation 2 destroyed !";
            Application.Quit();
        }
        else
        {
            RemTime.text = "Time Rem : " + (timeDelay2 - timeCtr).ToString("0.00");
        }
        ...
        if (PlayerControl.Enemy2IslandHit == 1 && flag == 0)
        {
            gobackflag = 0;
        ...
        if (flag == 1)
        {
            if (Time.time >= nextTime)
            {
                flag = 0;
                newTheta = 0;
                islandEnemy2.velocity = Vector3.zero;
                gobackflag = 1;
            ...
            {
                islandEnemy2.AddForce(pos * speed, ForceMode.Force);
            }
        }

        if (gobackflag == 1 && Vector3.Distance(transform.position, initPos) >= 2)
        {
            islandEnemy2.AddForce((initPos - islandEnemy2.transform.position) * goBackSpeed, ForceMode.Force);
        ...
        if ((Mathf.Abs(sub.transform.position.y - islandEnemy2.transform.position.y) <= 2) && sub.transform.position.y >= -4
            && DownwardForceFlag == 0 && Physics.Raycast(raytestGoDown, out hittestgd, 2) == false)
        {
            islandEnemy2.AddForce(new Vector3(0, -50000, 0), ForceMode.Force);
    ...
    /*private void OnDestroy()
    {
        if (HealthIsland2 <= 0)
        {
            PlayerControl.sw.WriteLine("enemy at island 2 destroyed");
            PlayerControl.sw.WriteLine("at position" + transform.position);
            PlayerControl.sw.WriteLine("at time" + Time.time);
        }
        else
        {
            PlayerControl.sw.WriteLine("enemy at island 2 survived");
        }
    }*/

    private void OnDestroy()
    {
        DestroyedAtPosIsl2 = transform.position;
        DestroyedAtTimeIsl2 = var;
    }
