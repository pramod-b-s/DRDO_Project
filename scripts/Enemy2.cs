...
public class Enemy2 : MonoBehaviour {
    ...
    public static Vector3 DestroyedAtPos2;
    public static float DestroyedAtTime2;
    private float var;
    ...
    void FixedUpdate()
    {
        ...
        if ((Mathf.Abs(sub.transform.position.y - enemy2.transform.position.y) <= 2) && sub.transform.position.y >= -4
            && DownwardForceFlag == 0 && Physics.Raycast(raytestGoDown, out hittestgd, 2) == false)
        {
            enemy2.AddForce(new Vector3(0, -50000, 0), ForceMode.Force);
        ...
        if (f == 1 || f1 == 1 || f2 == 1 || f3 == 1 || f4 == 1)
        {
            ...
            if (flagOther == 0)
            {
                    ...
                    && Physics.Raycast(rayr, out hitr, radius) == false)
                {
                    ...
                    enemy2.AddForce(pos * speed, ForceMode.Force);
        ...
        else
        {
            ...
            enemy2.AddForce((GameObject.FindGameObjectWithTag("sub_player").transform.position -
                GameObject.FindGameObjectWithTag("sub_enemy_2").transform.position) * enemySpeed, ForceMode.Force);

            float dist = Vector3.Distance(GameObject.FindGameObjectWithTag("sub_player").transform.position,
                GameObject.FindGameObjectWithTag("sub_enemy_2").transform.position);
            ...
            if (dist < tolerantDist)
            {
                ...
                if (dist < intolerabledist)
                {
                    ...
                    if (flag == 0)
                    {
                        myPos = GameObject.FindGameObjectWithTag("sub_enemy_2").transform.position;
                        initDist = dist;
        ...
        var = Time.time;
        //DODGE NOT ADDED AS THIS IS AGGRESSIVE SUBMARINE
    }

    private void OnDestroy()
    {
        DestroyedAtPos2 = transform.position;
        DestroyedAtTime2 = var;
    }
